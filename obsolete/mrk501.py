...
def read_mars(filename, tree='Events', leaves=[], verbose=False):
    ...
    while (nevents > 0):
        nevents -= 10000
        if (nevents > 0):
            tempdf = pd.DataFrame(events[nevents:(nevents + 10000)])
            del events[nevents:(nevents + 10000)]
        else:
            tempdf = pd.DataFrame(events[0:(nevents + 10000)])
...
if __name__ == '__main__':
    ...
    ax1 = plt.subplot(121)
    plt.title("MRK501 23.06.2014")
    plt.errorbar(x=bin_centers, y=flux_de, yerr=[flux_de_l, flux_de_h], fmt="o", label="FACT")
    plt.errorbar(x=hess_x * 1000, y=hess_y, yerr=[hess_yl, hess_yh], fmt=".", label="MRK501 HESS 23.06.2014")
