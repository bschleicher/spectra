...
def cutoff_powerlaw(x, k, gamma, Ec):
    k = k*10**(-11)
    return k*np.power(x/1000, gamma) * np.exp(np.divide(x, Ec))
...
def fit_ll(spect,
    ...
    if model == 'powerlaw':
        ...
        if bounds is None:
            bounds = [[10**(-2), 10**(4)], [-5, 1], [1000, 15000]]
        if names is None:
    ...
    def log_like_zd_e(A):
        start = 2
        stop = None
        y = spect.on_histo_zenith
        bsim = 0.2 * spect.off_histo_zenith
        ysim = countssim_zd_e(A) + bsim

        mask = ysim > 0
        mask2 = (bsim > 0) & (ysim > 0)
        b = np.sum(y[mask2] * np.log(bsim[mask2]) - bsim[mask2])
        s = np.sum(y[mask] * np.log(ysim[mask]) - ysim[mask])  # - np.log(factorial(y))
        return s - b
    ...
    return parameters, spec_function, bounds, names, sampler.chain